...
public class RecipeService {
    ...
    // HashMap til at opbevare opskrifter, hvor nøglen er recipeID, og værdien er et Recipe-objekt
    private final Map<Integer, Recipe> recipes = new HashMap<>();

    // Konstruktor til at initialisere opskrifter
    public RecipeService() {
        Ingredient egg = new Ingredient(1, "æg");
        Ingredient potato = new Ingredient(2, "kartoffel");
    ...
    // Find en opskrift baseret på ID
    public Recipe findRecipeById(int id) {
        return recipes.get(id);
    }

    // Filtrer opskrifter baseret på ingrediensnavn
    public List<Recipe> findRecipesByIngredient(String ingredientName) {
        return recipes.values().stream()
                .filter(recipe -> recipe.getIngredient().getIngredientName().equalsIgnoreCase(ingredientName))
    ...
    // Slet en opskrift baseret på ID
